# Seperate the code from the views so their wouldn't be to much code
...
def checkout(request, first_name, last_name, hostel_address, phone, email, amount):
    order = Order.objects.create(first_name=first_name, last_name=last_name, hostel_address=hostel_address, phone=phone,
                                 email=email, paid_amount=amount)
    
    for item in Cart(request):
        OrderItem.objects.create(order=order, product=item['product'], canteen=item['product'].canteen,
